...
$config["cooldown"]=2; // Vermek istediğiniz TİMEOUT YAZIN
// PREMİUM+ KONTROLÜ AKTİF ETMEK İSTİYORSAN BAŞINDAKİ // KALDIR

// if($seo_member==1){die(json_encode(array("status"=>"premium+"))); exit;} 


if($seo_member==1 || $seo_member==2){
    $sorgu++;
    $sqlekle="UPDATE `seotamsin_user` SET `sorgu` = \"$sorgu\" WHERE `id` = \"$id\"";
    $sonuc=mysqli_query($conn,$sqlekle);

    $text="**Sorgu Adı:** Soyağacı\n";
    $text.="**T.C.:** $tc\n";
    include("../seotamsin-server/lxg.php");

    if($seo_cooldown<=$current_date){
        $cooldown=strtotime("+".$config["cooldown"]." seconds", $currentDate);
        $sqlekle="UPDATE `seotamsin_user` SET `seo_cooldown` = \"$cooldown\" WHERE `id` = \"$id\"";
        $sonuc=mysqli_query($conn,$sqlekle);
    }else{
        die(json_encode(array("status"=>"cooldown", "saniye"=>$config["cooldown"])));
        exit;
    }
}
$str=$tc;
            ...
            // read data of each row
                    ...
                    while($row = $resultbabakardesi->fetch_assoc()) {
                        ...
                        while($row = $resultbabababasi->fetch_assoc()) {
                            ...
                            while($row = $resultbabaanasi->fetch_assoc()) {
                                $veri.= "<tr>
                                   <td> BABASININ BABASININ ANNESİ </td>
                                   <td>" . $row["TC"] . "</td>
                                   <td>" . $row["ADI"] . "</td>
                                   <td>" . $row["SOYADI"] . "</td>
                                   <td>" . $row["DOGUMTARIHI"] . "</td>
                        ...
                        while($row = $resultbabaanasi->fetch_assoc()) {
                            ...
                            while($row = $resultbabaanasi->fetch_assoc()) {
                                $veri.= "<tr>
                                   <td> BABASININ ANNESİNİN ANNESİ </td>
                                   <td>" . $row["TC"] . "</td>
                                   <td>" . $row["ADI"] . "</td>
                                   <td>" . $row["SOYADI"] . "</td>
                                   <td>" . $row["DOGUMTARIHI"] . "</td>
                ...
                while($row = $resultanasi->fetch_assoc()) {
                    ...
                    while($row = $resultannebabasi->fetch_assoc()) {
                        ...
                        while($row = $resultbabaanasi->fetch_assoc()) {
                            $veri.= "<tr>
                               <td> ANNESİNİN BABASININ ANNESİ </td>
                               <td>" . $row["TC"] . "</td>
                               <td>" . $row["ADI"] . "</td>
                               <td>" . $row["SOYADI"] . "</td>
                               <td>" . $row["DOGUMTARIHI"] . "</td>
                    ...
                    while($row = $resultanneanasi->fetch_assoc()) {
                        ...
                        while($row = $resultanneanasi->fetch_assoc()) {
                            $veri.= "<tr>
                               <td> ANNESİNİN ANNESİNİN ANNESİ </td>
                               <td>" . $row["TC"] . "</td>
                               <td>" . $row["ADI"] . "</td>
                               <td>" . $row["SOYADI"] . "</td>
                               <td>" . $row["DOGUMTARIHI"] . "</td>
